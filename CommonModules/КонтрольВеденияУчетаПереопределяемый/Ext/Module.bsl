...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	ГруппаДвиженияДокументов = ГруппыПроверок.Добавить();
	ГруппаДвиженияДокументов.Идентификатор = "ДвиженияДокументов";
	ГруппаДвиженияДокументов.Наименование = НСтр("ru='Движения документов'");
	
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы = ГруппаДвиженияДокументов.Идентификатор;
	Проверка.Идентификатор = "ДвиженияДокументов.НаличиеОтсутствиеКоличестваВЗаписяхРасходов";
	Проверка.Наименование = НСтр("ru='Проверка корректного наличия/отсутствия количества в движениях регистра накопления ""Расходы""'");
	Проверка.Причины = НСтр("ru='Была изменена единица измерения номенклатуры, уже используемой в проведенных документах ""Расход""'");
	Проверка.Рекомендация = НСтр("ru='Перепровести документ'");
	Проверка.ОбработчикПроверки = "РегистрыНакопления.Расходы.ПроверитьНаличиеОтсутствиеКоличестваВЗаписях";
